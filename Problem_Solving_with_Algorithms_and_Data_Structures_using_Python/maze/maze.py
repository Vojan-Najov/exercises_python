...
class Maze:
    def __init__(self, mazeFileName):
        ...
        self.wn.setworldcoordinates(-(columnsInMaze - 1) / 2 - 0.5,
                                    -(rowsInMaze - 1) / 2 - 0.5,
...
def searchFrom(maze, startRow, startColumn):
    # Пробуем идти из этой точки в каждом направлении, пока не найдем путь.
    # наружу. Возвращаемые значения для базового случая:
    #  1. Мы столкнулись с препятствием, возвращаем ложь
    maze.updatePosition(startRow, startColumn)
    if maze[startRow][startColumn] == OBSTACLE:
        return False
    #  2. Мы нашли квадрат, который уже был исследован
    if maze[startRow][startColumn] in (TRIED, DEAD_END):
        return False
